<x-app-layout>
    <x-slot name="header">
        {{ __('Edit News & Event') }}
    </x-slot>

    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <section class="p-8 mx-auto bg-white rounded-md shadow-md">
                
                <form action="{{ route('d.news.update', $news) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="w-28">
                        <x-toggle-switch name="status" checked="{{ $news->status }}">
                            Aktif
                        </x-toggle-switch>
                    </div>

                    <hr class="my-6 border-slate-200" />

                    <div class="grid grid-cols-1 gap-y-8">
                        <div>
                            <x-label for="title" class="mb-2">Judul</x-label>
                            <x-input id="title" type="text" name="title" class="w-full" value="{{ old('title', $news->title) }}" />

                            @error('title')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <div>
                            <x-label for="body" class="mb-2">Teks</x-label>
                            <input id="body" type="hidden" name="body" value="{{ old('body', $news->body) }}">
                            <trix-editor input="body"></trix-editor>

                            @error('body')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <div>
                            <x-label for="news_photo_path" class="mb-2">Cover</x-label>
                            <div class="h-44 mb-4">
                                <img src="{{ $news->getPhoto() }}" alt="{{ $news->title }}" class="h-full w-auto bg-cover rounded">
                            </div>
                            <x-input id="news_photo_path" type="file" name="news_photo_path" class="w-full" />

                            @error('news_photo_path')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="flex flex-col p-4 mt-4 border rounded-md">
                            <div class="mb-3">
                                <p class="font-light text-sm text-slate-500 italic">*Form/kontak pendaftaran untuk pelatihan (kosongkan jika tidak diperlukan)</p>
                            </div>

                            <div>
                                <div class="flex items-center">
                                    <x-input id="is_event" name="is_event" type="checkbox" value="1" checked="{{ $news->is_event ? 'checked' : '' }}" />
                                    <label for="is_event" class="ml-2 block text-sm font-medium text-slate-700">
                                        Aktifkan
                                    </label>
                                </div>

                                @error('is_event')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>

                            <div class="mt-6">
                                <x-label for="contact" class="mb-2">Whatsapp</x-label>
                                <x-input id="contact" type="tel" name="contact" class="w-full px-4 py-2" placeholder="628123456789" value="{{ old('contact', $news->contact) }}" />
    
                                @error('contact')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center mt-12">
                        <a href="{{ route('d.news.index') }}" class="mr-2 btn-secondary">Batal</a>
                        <x-button>Simpan</x-button>
                    </div>
                </form>
            </section>

        </div>
    </div>
</x-app-layout>
