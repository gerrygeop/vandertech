<div class="col-span-1">
    <a href="{{ $link }}">
        <div class="bg-white p-4 rounded-lg group hover:-translate-y-2 hover:shadow-lg overflow-hidden transition duration-200">
            <img class="h-40 rounded w-full object-cover object-center {{ $isEvent ? 'mb-3' : 'mb-9' }}" src="{{ $cover }}" alt="content">

            @if ($isEvent)
                <span class="tracking-widest text-indigo-500 text-xs font-medium uppercase">Pelatihan</span>
            @endif
            ...
            <p class="inline-flex items-center -mx-1 mt-2 text-sm text-blue-500 capitalize transition duration-200 dark:text-blue-400 group-hover:underline group-hover:text-blue-600 dark:group-hover:text-blue-500">
